...
6. **Add the three values and turn it on**: in your worker → **Settings → Variables and Secrets** → the three placeholders are listed empty — click **Edit** on each and paste `BOT_TOKEN` (step 1), `GEMINI_API_KEY` (step 2), `ALLOWED_CHAT_ID` (step 3) → **Deploy** → go to your worker's **Domains** tab and enable the **Worker URL** — the **Visit** button appears → click it.
